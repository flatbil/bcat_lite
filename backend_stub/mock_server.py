# backend_stub/mock_server.py — Multi-building campus rooms & schedules
...
# ── Room data (keyed by building_id) ─────────────────────────────────────────
_rooms: dict = {
    "school_main": {
        # ── North Wing ──────────────────────────────────────────────────────
        "bay_747": {
        ...
        # ── First Floor ───────────────────────────────────────────────────
        "f1_admin": {
            "id": "f1_admin", "name": "Administration Suite", "capacity": 15,
            "available": True,
            "meetings_today": [
                {"time": "08:00–09:00", "title": "Weekly Admin Team Standup",      "organizer": "Head Admin"},
                {"time": "14:00–15:00", "title": "Budget Review",                  "organizer": "Finance Office"},
            ],
        },
        "f1_itp": {
            "id": "f1_itp", "name": "IT Department", "capacity": 20,
            "available": True,
            "meetings_today": [
                {"time": "08:30–09:30", "title": "Network Maintenance Window",     "organizer": "IT Manager"},
                {"time": "13:00–14:00", "title": "Device Inventory Audit",         "organizer": "IT Manager"},
            ],
        },
        "f1_seminar": {
            "id": "f1_seminar", "name": "Seminar Room", "capacity": 35,
            "available": False,
            "meetings_today": [
                {"time": "09:00–11:00", "title": "Staff Development Day — Session 1", "organizer": "Principal Adams"},
                {"time": "13:00–15:00", "title": "Staff Development Day — Session 2", "organizer": "Principal Adams"},
            ],
        },
    },
    "annex_b": {
        "anx_workshop": {
            "id": "anx_workshop", "name": "Workshop", "capacity": 25,
            "available": True,
            "meetings_today": [
                {"time": "09:00–11:30", "title": "Woodworking — Grade 10",         "organizer": "Mr. Santos"},
            ],
        },
        "anx_storage": {
            "id": "anx_storage", "name": "Material Storage", "capacity": 5,
            "available": True,
            "meetings_today": [],
        },
        "anx_lab": {
            "id": "anx_lab", "name": "Vocational Lab", "capacity": 30,
            "available": False,
            "meetings_today": [
                {"time": "08:00–09:30", "title": "Auto Tech — Grade 11",           "organizer": "Ms. Park"},
                {"time": "10:00–11:30", "title": "Auto Tech — Grade 12",           "organizer": "Ms. Park"},
            ],
        },
    },
}
...
def get_building_location():
    ...
    Also returns raw lat/lon for campus building auto-detection."""
    ...
    dz = -((_gps_state["lat"] - anchor_lat) * 111319.5)
    x = _gps_anchor["bx"] + dx
    z = _gps_anchor["bz"] + dz
    return {
        "has_fix": True,
...
@app.get("/rooms")
def list_rooms(building: str = "school_main"):
    """Return all rooms for a building.  ?building=<id>"""
    if building not in _rooms:
        return {}
    return _rooms[building]


@app.get("/rooms/{room_id}")
def get_room(room_id: str, building: str = "school_main"):
    brooms = _rooms.get(building, {})
    if room_id not in brooms:
        raise HTTPException(404, "Room not found")
    return brooms[room_id]


@app.post("/rooms/{room_id}/toggle")
def toggle_room(room_id: str, building: str = "school_main"):
    """Flip availability — handy for live demo."""
    brooms = _rooms.get(building, {})
    if room_id not in brooms:
        raise HTTPException(404, "Room not found")
    brooms[room_id]["available"] = not brooms[room_id]["available"]
    return brooms[room_id]


@app.post("/calendar/schedule")
def get_schedule(payload: ScheduleRequest):
    """Stub that mirrors Microsoft Graph /me/calendar/getSchedule response shape.
    Returns mock free/busy slots for each requested calendar_id."""
    mock_slots = []
    for cal_id in payload.calendar_ids:
        mock_slots.append({
            "scheduleId": cal_id,
            "availabilityView": "0200000002000000",   # '0'=free, '2'=busy
            "scheduleItems": [
                {
                    "status": "busy",
